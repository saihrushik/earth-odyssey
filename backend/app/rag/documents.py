"""Build the knowledge corpus from the shared dataset (port of documents.ts)."""
...
GUIDES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "tags": ["photography", "sunrise", "golden hour", "sunset"],
...
def build_knowledge_base() -> list[dict[str, Any]]:
    """Destination docs (3 facets each) + general guides — same shape as the TS build."""
    docs: list[dict[str, Any]] = []
    for d in data.destinations():
        unesco = "It is a UNESCO World Heritage site." if d["unesco"] else ""
        docs.append(
            {
                "id": f"{d['id']}:overview",
                "destinationId": d["id"],
                "title": f"{d['name']} — overview",
                "tags": d["tags"],
                "text": f"{d['name']} ({d['country']}, {d['region']}). {d['tagline']}. {d['history']} "
                f"Notable facts: {' '.join(d['facts'])} {unesco}",
            }
        )
        b = d["budgetPerDay"]
        docs.append(
            {
                "id": f"{d['id']}:practical",
                "destinationId": d["id"],
                "title": f"{d['name']} — practical guide",
                "tags": [*d["tags"], "visa", "budget", "season", "safety", "transport"],
                "text": f"Practical guide for {d['name']}, {d['country']}. Best season: {d['bestSeason']}. "
                f"Daily budget in USD: backpacker ${b['backpacker']}, mid-range ${b['midrange']}, luxury ${b['luxury']}. "
                f"Suggested trip length: {d['tripDays']}. Visa: {d['visa']} Language: {d['language']}. "
                f"Currency: {d['currencyName']} ({d['currency']}). Safety: {d['safety']} "
                f"Getting there and around: {d['transport']}",
            }
        )
        docs.append(
            {
                "id": f"{d['id']}:experience",
                "destinationId": d["id"],
                "title": f"{d['name']} — experiences, food and hidden gems",
                "tags": [*d["tags"], "food", "attractions", "hidden gems"],
                "text": f"What to do in {d['name']}: {'; '.join(d['attractions'])}. "
                f"Local food to try: {'; '.join(d['food'])}. Hidden gems: {'; '.join(d['hiddenGems'])}. "
                f"Nearby: {'; '.join(d['nearby'])}. Travel styles it suits: {', '.join(d['tags'])}.",
            }
        )
    docs.extend(GUIDES)
    return docs
